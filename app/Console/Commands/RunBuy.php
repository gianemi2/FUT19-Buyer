...
class RunBuy extends Command {
    ...
    */
    ...
    public function handle() {
        ...
        $start_at = '00:00:00';
        $end_at = '23:59:00';
        // if(Carbon::now() > Carbon::parse($start_at) && Carbon::now() < Carbon::parse($end_at) ){
        ...
        try {
            $this->fut = new Core(
                ...
                if($this->account->cooldown == 1) {
                    if(Carbon::now()->diffInMinutes(Carbon::parse($this->account->cooldown_activated)) > Setting::get('account_cooldown')) {
                        ...
                        $telegram->sendMessage('*'.Setting::get('account_name').'* has been removed from cooldown!');
                        abort(200);
                    } else {
                        Accounts::find($this->account->id)->update([
                            'in_use' => '0'
                    ...
                    if(Carbon::now()->diffInMinutes(Carbon::parse($this->account->last_login)) > Setting::get('account_runtime')) {
                        ...
                        $this->fut->logout();
                        $telegram->sendMessage('*'.Setting::get('account_name').'* has been placed in cooldown!');
                        abort(403);
                    }
                }
                                        ...
                                        if(count($search['auctionInfo']) < 8) {
                                            foreach($search['auctionInfo'] as $auction) {
                                                ...
                                                try {
                                                    ...
                                                    $error = $e->GetOptions();
                                                    Log::notice('We failed to buy the auction for '.$player->name.' after attempting to buy him for '.$auction['buyNowPrice'] . '. Reason: '.$error['reason']);
                                                    $auctionsFailed++;
                                                }
                                            }
                                        } else {
                ...
                $telegram->sendMessage('*'.Setting::get('account_name').'* has caught an exception! '. $error['reason']);
            ...
            // } else{
            //     Log::info('Is '. Carbon::now() . '. Im going to have a rest.');
            //     abort(403);
            // }
        ...
        private function sort_item_list()
        {
            try {
                ...
                // $telegram->sendMessage('*'.Setting::get('account_name').'* has caught an exception in transfer list! - ' . $error['reason']);
